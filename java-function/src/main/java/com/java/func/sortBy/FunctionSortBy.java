...
public class FunctionSortBy {
    ...
    public static void main(String[] args)
        ...
        DomainUser domainUser1 = DomainUser.builder().id(11L).name("tony").sex(Boolean.FALSE).age(10).build();

        DomainUser domainUser2 = DomainUser.builder().id(41L).name("Test").sex(Boolean.TRUE).age(50).build();

        DomainUser domainUser3 = DomainUser.builder().id(14L).name("Mark").sex(Boolean.FALSE).age(34).build();
        ...
        List<DomainUser> collect =
            domainUsers.stream().sorted((u1, u2) -> extracted(orderBy, u1, u2)).collect(Collectors.toList());
    ...
    private static int extracted(String orderBy, DomainUser u1, DomainUser u2) {
        ...
        try {
            ...
            if ("String".equals(className)) {
                ...
                compareTo = Long.compare(Long.parseLong(method.invoke(u1).toString()),
                    Long.parseLong(method.invoke(u2).toString()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
