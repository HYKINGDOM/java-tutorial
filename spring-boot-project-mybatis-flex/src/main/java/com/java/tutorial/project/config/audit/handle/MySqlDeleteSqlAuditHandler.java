...
public class MySqlDeleteSqlAuditHandler extends AbstractSQLAuditHandler {
    ...
    public MySqlDeleteSqlAuditHandler(Connection connection, DBMetaDataHolder dbMetaDataHolder, Method clerkIdMethod,
        String sql) {
    ...
    protected SQLTableSource getMajorTableSource(SQLStatement statement) {
        if (statement instanceof MySqlDeleteStatement) {
            return ((MySqlDeleteStatement)statement).getFrom() != null ? ((MySqlDeleteStatement)statement).getFrom()
                : ((MySqlDeleteStatement)statement).getTableSource();
        } else {
            return null;
        }
    }
    ...
    public void preHandle() {
        if (getSqlStatement() instanceof MySqlDeleteStatement) {
            ...
            SQLTableSource affectTableSource =
                deleteStatement.getTableSource() != null ? deleteStatement.getTableSource() : deleteStatement.getFrom();
            List<String> affectAliasList = buildTableSourceAliases(affectTableSource);
            SQLTableSource from =
                deleteStatement.getFrom() != null ? deleteStatement.getFrom() : deleteStatement.getTableSource();
            ...
            for (String alias : affectAliasList) {
                selectQueryBlock.getSelectList().add(new SQLSelectItem(SQLUtils.toSQLExpr(String.format("%s.%s", alias,
                    getDbMetaDataHolder().getPrimaryKeys().get(getAliasToTableMap().get(alias))))));
                ...
                for (int i = 1; i < columnCount + 1; i++) {
                    ...
                    if (StrUtil.isNotBlank(tableName)) {
                        ...
                        if (list.size() <= row) {
                            ...
                            cols.add(new AuditLog(tableName,
                                getDbMetaDataHolder().getTableColumns().get(tableName).get(i - 2), null,
                                primaryKeyMap.get(tableName), AuditOperationEnum.delete.name(), resultSet.getObject(i),
                                null));
                        }
