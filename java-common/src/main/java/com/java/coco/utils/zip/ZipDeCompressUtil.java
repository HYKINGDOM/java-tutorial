...
public abstract class ZipDeCompressUtil {
     ...
     */
    public static String deCompress(String srcFilePath, String destDirPath, String charset, String passwd)
        ...
        try (ZipFile zFile = new ZipFile(srcFile)) {
     ...
     * @param destDirPath 解压后路径
     */
    public static String deCompress(String srcFilePath, String destDirPath, String password) throws RuntimeException {
        File srcFile = new File(srcFilePath);
        if (!srcFile.exists()) {
            log.error("压缩文件{}不存在", srcFilePath);
            throw new RuntimeException("压缩文件不存在");
        }
        return deCompress(srcFilePath, destDirPath, CHARSET_UTF8, password);
    }
     ...
     * @param srcFilePath 压缩文件源路径
     */
    public static String deCompress(String srcFilePath, String password) throws RuntimeException {
        File srcFile = new File(srcFilePath);
        if (!srcFile.exists()) {
            log.error("压缩文件{}不存在", srcFilePath);
            throw new RuntimeException("压缩文件不存在");
        ...
        return deCompress(srcFilePath, destDirPath, password);
    }
