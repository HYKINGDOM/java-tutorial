...
public class MessageDtoBuilder {
    ...
    public static MessageDto builderMessageDto(Long businessId, Object payload, String version) {
        return MessageDto.builder().businessId(businessId).payload(payload).version(version)
            .traceId(TraceIDUtil.getTraceId()).currentDateTime(LocalDateTime.now()).build();
    }

    /**
     * 将消息的消费端接收的消息进行转换 转换过程中设置TraceID
     *
     * @param messageObject
     * @return
     */
