...
import com.java.demo.listener.AssemblyReadCsvPathListener;
import com.java.demo.listener.CustomStepExecutionListener;
import com.java.demo.mapper.ReoportFieldSetMapper;
import com.java.demo.repository.AddressRepository;
import com.java.demo.tasklet.PrintImportFilePathTaskLet;
...
public class ImportPersonJobConfig {
    ...
    public FlatFileItemReader<Address> readCsvItemReader(
        ...
        // 读取数据
        return new FlatFileItemReaderBuilder<Address>().name("read-csv-file")
            .resource(new ClassPathResource(importPath)).delimited().delimiter(",")
            .names("address_id", "address", "address2", "district", "city_id", "postal_code", "phone", "last_update")
            .linesToSkip(1).fieldSetMapper(new ReoportFieldSetMapper()).build();
    }
