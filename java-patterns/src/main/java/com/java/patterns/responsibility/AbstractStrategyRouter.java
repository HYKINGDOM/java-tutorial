...
/**
 * 通用的“策略树“框架，通过树形结构实现分发与委托，每层通过指定的参数进行向下分发委托，直到达到最终的执行者。 该框架包含两个类：{@code StrategyHandler} 和
 * {@code AbstractStrategyRouter} 其中：通过实现 {@code AbstractStrategyRouter} 抽象类完成对策略的分发， 实现 {@code StrategyHandler}
 * 接口来对策略进行实现。 像是第二层 A、B 这样的节点，既是 Root 节点的策略实现者也是策略A1、A2、B1、B2 的分发者，这样的节点只需要 同时继承 {@code StrategyHandler} 和实现
 * {@code AbstractStrategyRouter} 接口就可以了。
 *
 * <pre>
 *           +---------+
 *           |  Root   |   ----------- 第 1 层策略入口
