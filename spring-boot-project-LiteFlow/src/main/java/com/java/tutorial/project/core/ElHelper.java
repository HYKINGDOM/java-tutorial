package com.java.tutorial.project.core;

import com.google.common.collect.Lists;
import com.google.common.collect.Sets;
import com.java.tutorial.project.core.enums.NodeEnum;
import com.java.tutorial.project.core.factory.NodeFactory;
import com.java.tutorial.project.core.function.NodeFunction;
import com.java.tutorial.project.core.model.JsonAstModel;
...
public class ElHelper {
    ...
    private static ThenELWrapper tree2El(Node currentNode, ThenELWrapper currentThenElWrapper,
        Deque<ThenELWrapper> stack, Set<String> doneSummary) {
        NodeFunction.dealNode(currentNode.getNodeEnum(),
            ...
            () -> {
                ...
                ELWrapper ignore = NodeEnum.SUMMARY.equals(convert2IfNode(currentNode).getFalseNode().getNodeEnum())
                    ...
                    : currentThenElWrapper.then(
                        ELBus.ifOpt(currentNode.getName(), trueThenElWrapper, falseThenElWrapper));
    ...
    public static Node Json2Node(JsonAstModel model) {
        ...
        // 创建node
        Map<String, Node> nodeMap = nodeEntities.stream().map(
            nodeEntity -> NodeFactory.getNode(NodeEnum.valueByName(nodeEntity.getNodeType()), nodeEntity.getId(),
                nodeEntity.getName())).collect(Collectors.toMap(Node::getId, Function.identity()));
        // 构建节点关系
        for (JsonAstModel.NodeEdge nodeEdge : nodeEdges) {
            ...
            // 不同节点处理方式不同
            NodeFunction.dealNode(sourceNode.getNodeEnum(), () -> sourceNode.addNextNode(targetNode),
                ...
                () -> sourceNode.addNextNode(targetNode), () -> sourceNode.addNextNode(targetNode),
                () -> sourceNode.addNextNode(targetNode));
            // 设置前置节点
            targetNode.addPreNode(sourceNode);
        }
        // 只能有一个开始结点
        List<Node> start = nodeMap.values().stream().filter(node -> NodeEnum.START.equals(node.getNodeEnum()))
            .collect(Collectors.toList());
        if (start.size() != 1) {
            throw new RuntimeException("语法树不合法，有且只能有一个开始结点！");
        }
        // 只能有一个结束结点
        List<Node> end = nodeMap.values().stream().filter(node -> NodeEnum.END.equals(node.getNodeEnum()))
            .collect(Collectors.toList());
        if (end.size() != 1) {
        ...
        for (Node node : nodeMap.values()) {
            // COMMON节点的出度和入度只能是1
            if (NodeEnum.COMMON.equals(node.getNodeEnum()) && (node.getPre().size() != 1 || node.getNext()
                .size() != 1)) {
                throw new RuntimeException("语法树不合法，普通节点的出度和入度不能大于1");
            }
            // 开始节点出度为1，结束节点入度为1
            if (NodeEnum.START.equals(node.getNodeEnum()) && (!node.getPre().isEmpty() || node.getNext()
                .size() != 1)) {
                throw new RuntimeException("语法树不合法，开始节点的出度只能为1、入度只能为0");
            }
            if (NodeEnum.END.equals(node.getNodeEnum()) && (node.getPre().size() != 1 || !node.getNext().isEmpty())) {
    ...
    private static void checkJsonAst(JsonAstModel model) {
        ...
        nodeEntities.forEach(nodeEntity -> {
            if (StringUtils.isAnyBlank(nodeEntity.getId(), nodeEntity.getLabel(), nodeEntity.getName(),
                nodeEntity.getNodeType()) || nodeEntity.getX() == null || nodeEntity.getY() == null) {
        ...
        nodeEdges.forEach(nodeEdge -> {
            if (StringUtils.isAnyBlank(nodeEdge.getSource(), nodeEdge.getTarget())) {
                throw new RuntimeException(
                    "必填参数校验不通过，边的基本属性缺失：" + nodeEdge.getSource() + "->" + nodeEdge.getTarget());
            }
        });
        ...
        for (JsonAstModel.NodeEntity nodeEntity : nodeEntities) {
            ...
            if (NodeEnum.IF.equals(nodeEnum)) {
                List<JsonAstModel.NodeEdge> thisNodeEdges =
                    nodeEdges.stream().filter(nodeEdge -> nodeEntity.getId().equals(nodeEdge.getSource()))
                        .collect(Collectors.toList());
                ...
                if (firstNodeFlag == null || secondNodeFlag == null || firstNodeFlag.equals(secondNodeFlag)) {
                    throw new RuntimeException(
                        "语法树不合法，if节点必须有一个true分支和一个false分支：" + nodeEntity.getLabel());
                }
            }
