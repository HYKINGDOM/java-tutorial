...
public final class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> systemException(SystemException ex) {
        return ResponseEntity.status(HttpStatus.NOT_EXTENDED).header("Content-Type", "text/plain;charset=UTF-8")
            .body(ex.getMessage());
    }
    ...
    public ResponseEntity<String> bindException(ConstraintViolationException ex) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).header("Content-Type", "text/plain;charset=UTF-8")
            .body(error);
    }
