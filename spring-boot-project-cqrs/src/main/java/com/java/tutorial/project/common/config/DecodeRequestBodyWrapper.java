...
public final class DecodeRequestBodyWrapper extends HttpServletRequestWrapper {
    ...
    /**
     * 1、拿到request body的 input stream 2、将输入流转成字符串（用AES加密的密文字符串） 3、拿到头信息里的AES秘钥（用RSA加密的AES秘钥） 4、用RSA解密AES秘钥，获得AES秘钥明文
     * 5、用AES秘钥解密密文body 6、将明文转成ServletInputStream
     */
    @Override
    public ServletInputStream getInputStream() throws IOException {
        ...
        if (StrKit.notBlank(plaintextBody)) {
            final ByteArrayInputStream stream =
                new ByteArrayInputStream(plaintextBody.getBytes(StandardCharsets.UTF_8));
